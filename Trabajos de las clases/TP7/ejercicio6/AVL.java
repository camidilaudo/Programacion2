...
public class AVL<E extends Comparable<E>> implements AVLTDA<E> {
    ...
    public void levelOrder() {
        ...
        LinkedQueue<NodoAVL<E>> queue = new LinkedQueue<>();
        queue.enqueue(root);
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < levelSize; i++) {
                NodoAVL<E> current = queue.dequeue();
    ...
    private int height(NodoAVL<E> node) {
    ...
    public void inOrderToList(LinkedList<E> list) {
        inOrderToList(root, list);
    }

    private void inOrderToList(NodoAVL<E> node, LinkedList<E> list) {
        if (node != null) {
